...
import { onClose, onOpen } from '../auth-menu/hook'
            ...
            <div className='flex py-2 px-4 bg-white xl:max-w-[1366px] m-auto'>
                ...
                <div className='flex'>
                    ...
                    <div
                        ...
                        className="w-9 h-9 cursor-pointer rounded-full flex items-center justify-center ml-2 bg-slate-100 relative group">
                        {profile?.avatar ?
                            <Image priority className="rounded-full object-contain shadow" src={`${DB_HOST}/${profile.avatar}`} alt='' width={40} height={40} />
                            :
                            <i className="fa-solid fa-user text-xl"></i>
                        }
                        <div className="absolute text-center top-12 right-0 min-w-max bg-slate-300 text-slate-700 text-sm px-2 py-1 rounded mt-2 opacity-0 group-hover:opacity-100 transition duration-300">
                            Tài khoản
                        </div>
                    </div>
                </div>
